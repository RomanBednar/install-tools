...
ENTER_CMD = ${engine} run --cap-add SYS_ADMIN --privileged -it -v ${CURRENT_DIR}:/code -v ${homeDir}:/root -v /var/run/docker.sock:/var/run/docker.sock localhost/${imageTag} /bin/bash
